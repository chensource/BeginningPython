...
import FangLink.fetch_free_proxyes

logger = logging.getLogger(__name__)


class FanglinkSpiderMiddleware(object):
    ...
    def inc_proxy_index(self):
        ...
        ) < self.last_no_proxy_time + timedelta(minutes=2):
            logger.info("captcha thrashing")
            self.fetch_new_proxyes()

        if self.len_valid_proxy() <= self.fixed_proxy or self.len_valid_proxy(
        ) < self.extend_proxy_threshold:  # 如果代理列表中有效的代理不足的话重置为valid
            self.reset_proxyes()

        if self.len_valid_proxy(
        ) < self.extend_proxy_threshold:  # 代理数量仍然不足, 抓取新的代理
    ...
    def process_request(self, request, spider):
        ...
        if self.proxy_index > 0 and datetime.now() > (
                ...
                timedelta(minutes=self.recover_interval)):
        ...
        # print("USE PROXY -> %s" % request.meta["proxy_index"])

        # spider发现parse error, 要求更换代理
        if "change_proxy" in request.meta.keys(
        ) and request.meta["change_proxy"]:
    ...
    def process_response(self, request, response, spider):
        ...
        if response.status != 200 and (
                not hasattr(spider, "website_possible_httpstatus_list") or
    ...
    def process_exception(self, request, exception, spider):
        ...
        logger.debug("%s exception: %s" %
                     (self.proxyes[request.meta["proxy_index"]]["proxy"],
                      exception))
